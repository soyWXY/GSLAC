...
std::chrono::high_resolution_clock::time_point init_start, init_end, sync_start,
    sync_end, zeroing_start, zeroing_end, transfer_start, transfer_end,
    total_start, total_end;
std::chrono::duration<double> init_elapsed_seconds, sync_elapsed_seconds,
    zeroing_elapsed_seconds, transfer_elapsed_seconds, total_elapsed_seconds;
...
int main(int argc, char **argv) {
    ...
    while ((clnt_sock = accept(serv_sock, (struct sockaddr *)&clnt_addr,
        ...
        if (strcmp(buf, "0001") == 0) {
            ...
            cuInit = reinterpret_cast<CUresult (*)(unsigned int)>(
                dlsym(handle, "cuInit"));
            CUresult err;
            err = cuInit(flags);
            ...
            cuDeviceGet = reinterpret_cast<CUresult (*)(CUdevice *, int)>(
            ...
            cuCtxCreate = reinterpret_cast<CUresult (*)(
                CUcontext *, unsigned int, CUdevice)>(
                dlsym(handle, "cuCtxCreate"));
            ...
            cuCtxGetApiVersion =
                reinterpret_cast<CUresult (*)(CUcontext, unsigned int *)>(
                    dlsym(handle, "cuCtxGetApiVersion"));
            ...
            cuModuleLoad =
                reinterpret_cast<CUresult (*)(CUmodule *, const char *)>(
                    dlsym(handle, "cuModuleLoad"));
            ...
            cuModuleGetFunction = reinterpret_cast<CUresult (*)(
                CUfunction *, CUmodule, const char *)>(
                dlsym(handle, "cuModuleGetFunction"));

            CUresult err;
            if (atoi(argv[1]) == 1)
                err = cuModuleGetFunction(&vecAdd_kernel, cuModule,
                                          "VecAdd_kernel");
            ...
            std::cout << "init elapsed time: " << init_elapsed_seconds.count()
                      << "s" << std::endl;
            ...
            cuMemAlloc = reinterpret_cast<CUresult (*)(CUdeviceptr *, size_t)>(
                dlsym(handle, "cuMemAlloc"));
            cuMemsetD32 = reinterpret_cast<CUresult (*)(CUdeviceptr *,
                                                        unsigned int, size_t)>(
                dlsym(handle, "cuMemsetD32"));
            ...
            if (!d_A) {
                ...
                zeroing_end = std::chrono::system_clock::now();
                zeroing_elapsed_seconds += zeroing_end - zeroing_start;
                std::cout << "zeroing elapsed time: "
                          << zeroing_elapsed_seconds.count() << "s"
                          << std::endl;
            }
            ...
            cuMemcpyHtoD = reinterpret_cast<CUresult (*)(CUdeviceptr,
                                                         const void *, size_t)>(
                dlsym(handle, "cuMemcpyHtoD"));
            ...
            if (atoi(argv[1]) == 1) args = args1;

            if (atoi(argv[1]) == 2) args = args2;

            cuLaunchKernel = reinterpret_cast<CUresult (*)(
                CUfunction, unsigned int, unsigned int, unsigned int,
                unsigned int, unsigned int, unsigned int, unsigned int,
                CUstream, void **, void **)>(dlsym(handle, "cuLaunchKernel"));

            unsigned int gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY,
                blockDimZ;
            ...
            err = cuLaunchKernel(vecAdd_kernel, gridDimX, gridDimY, gridDimZ,
                                 blockDimX, blockDimY, blockDimZ, 0, NULL, args,
                                 NULL);
            send(clnt_sock, &err, sizeof(CUresult), 0);
            ...
            cuCtxSynchronize = reinterpret_cast<CUresult (*)()>(
            ...
            std::cout << "sync elapsed time: " << sync_elapsed_seconds.count()
                      << "s" << std::endl;
            ...
            transfer_end = std::chrono::system_clock::now();
            transfer_elapsed_seconds += transfer_end - transfer_start;
            std::cout << "transfer elapsed time: "
                      << transfer_elapsed_seconds.count() << "s" << std::endl;
            ...
            std::cout << "total elapsed time: " << total_elapsed_seconds.count()
                      << "s" << std::endl;
        } else {
            printf("Exception\n");
        }
